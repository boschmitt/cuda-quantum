...
static Value createGlobalCString(Operation *op, Location loc,
  ...
  Value nameVal = cudaq::cc::AddressOfOp::create(rewriter, loc, cudaq::cc::PointerType::get(nameObj.getType()), nameObj.getName());
  ...
  matchAndRewrite(quake::AllocaOp alloc, OpAdaptor adaptor,
    ...
    if (adaptor.getOperands().empty()) {
      ...
      if (sizeOpTy.getWidth() < 64)
        sizeOperand = cudaq::cc::CastOp::create(rewriter, loc, rewriter.getI64Type(), sizeOperand,
  ...
  matchAndRewrite(quake::ApplyNoiseOp noise, OpAdaptor adaptor,
    ...
    if (!noise.getNoiseFunc()) {
      ...
      if (pushASpan) {
        ...
        args.push_back(cudaq::cc::StdvecSizeOp::create(rewriter, loc, rewriter.getI64Type(), stdvec));
      } else {
    ...
    if (adaptor.getParameters().size() == 1 &&
      ...
      Value endPtr = cudaq::cc::ComputePtrOp::create(rewriter, loc, ptrTy, startPtr, ArrayRef<cudaq::cc::ComputePtrArg>{len});
      Value castStartPtr =
          cudaq::cc::CastOp::create(rewriter, loc, ptrTy, startPtr);
      auto ptrPtrTy = cudaq::cc::PointerType::get(ptrTy);
      Value ptr0 = cudaq::cc::ComputePtrOp::create(rewriter, loc, ptrPtrTy, hostVec, ArrayRef<cudaq::cc::ComputePtrArg>{0});
      cudaq::cc::StoreOp::create(rewriter, loc, castStartPtr, ptr0);
      Value ptr1 = cudaq::cc::ComputePtrOp::create(rewriter, loc, ptrPtrTy, hostVec, ArrayRef<cudaq::cc::ComputePtrArg>{1});
      cudaq::cc::StoreOp::create(rewriter, loc, endPtr, ptr1);
      Value ptr2 = cudaq::cc::ComputePtrOp::create(rewriter, loc, ptrPtrTy, hostVec, ArrayRef<cudaq::cc::ComputePtrArg>{2});
    ...
    for (auto [qb, oa] : llvm::zip(adaptor.getQubits(), noise.getQubits())) {
      if ((oa && isa<quake::VeqType>(oa.getType())) ||
        ...
        auto svec = func::CallOp::create(rewriter, loc, qirArrTy, cudaq::opt::QISConvertArrayToStdvec, ValueRange{qb});
        qb = svec.getResult(0);
  ...
  matchAndRewrite(cudaq::codegen::RAIIOp raii, OpAdaptor adaptor,
    ...
    if (!adaptor.getAllocSize()) {
      ...
      if (sizeTy.getWidth() < 64)
        sizeOperand = cudaq::cc::CastOp::create(rewriter, loc, i64Ty, sizeOperand, cudaq::cc::CastOpMode::Unsigned);
    ...
    auto extend = [&](Value &v) -> Value {
      if (auto intTy = dyn_cast<IntegerType>(v.getType())) {
        if (intTy.getWidth() < 64)
          return cudaq::cc::CastOp::create(rewriter, loc, i64Ty, v, cudaq::cc::CastOpMode::Unsigned);
        if (intTy.getWidth() > 64)
...
struct CustomUnitaryOpPattern
  ...
  matchAndRewrite(quake::CustomUnitarySymbolOp unitary, quake::CustomUnitarySymbolOp::Adaptor adaptor,
    ...
    auto addrOp = cudaq::cc::AddressOfOp::create(rewriter, loc, globalObj.getType(), generatorName);
    auto unitaryData = cudaq::cc::CastOp::create(rewriter, loc, complex64PtrTy, addrOp);
