...
void extractKrausData(const nb::ndarray<nb::numpy, const std::complex<double>> &array, complex *data) {
  if (array.ndim() != 2)
  ...
  auto strides = Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>(
      array.stride(rowMajor ? 0 : 1) / sizeof(Scalar),
      array.stride(rowMajor ? 1 : 0) / sizeof(Scalar));
  auto map =
      Eigen::Map<RowMajorMat, 0, Eigen::Stride<Eigen::Dynamic, Eigen::Dynamic>>(
          const_cast<Scalar *>(array.data()), array.shape(0), array.shape(1),
          strides);
  RowMajorMat eigenMat(map);
  memcpy(data, eigenMat.data(),
         sizeof(complex) * (array.shape(0) * array.shape(1)));
}
...
void bindNoiseModel(nb::module_ &mod) {
  ...
  nb::class_<noise_model>(
      ...
      .def("__init__", [](noise_model &self, nb::module_ mod) {
             ...
             for (const auto &[name, generator] : channelGenerators) {
               if (nb::hasattr(mod, name.c_str())) {
                 nb::object channelType = nb::getattr(mod, name.c_str());
      ...
      .def(
          "register_channel",
          [](noise_model &self, const nb::object &krausT) {
            ...
            std::function<kraus_channel(const std::vector<double> &)> lambda =
              ...
              auto instance = krausT(p);
              return nb::cast<kraus_channel>(instance);
            };
...
void bindKrausOp(nb::module_ &mod) {
  nb::class_<kraus_op>(
      ...
      .def("__init__", [](kraus_op *self, const nb::ndarray<nb::numpy, const std::complex<double>> &array) {
             std::vector<complex> v(array.shape(0) * array.shape(1));
             extractKrausData(array, v.data());
             new (self) kraus_op(v);
...
void bindNoiseChannels(nb::module_ &mod) {
  ...
  nb::class_<kraus_channel, PyKrausChannel>(
      ...
      .def("__init__", [](kraus_channel *self, nb::list ops) {
             std::vector<kraus_op> kops;
             for (std::size_t i = 0; i < ops.size(); i++) {
               auto array = nb::cast<nb::ndarray<nb::numpy, const std::complex<double>>>(ops[i]);
               auto shape0 = array.shape(0);
               auto shape1 = array.shape(1);
               std::vector<complex> v(shape0 * shape1);
               extractKrausData(array, v.data());
               kops.emplace_back(v);
             }
