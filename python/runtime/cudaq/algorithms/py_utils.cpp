  ...
  auto try_to_add_item = [&](const auto item) {
    try {
      ...
      if (nb::cast<std::string>(key).starts_with("__")) {
        ...
        std::string module =
            nb::cast<std::string>(type.attr("__module__"));
        std::string name = nb::cast<std::string>(type.attr("__name__"));
        std::string type_name = module + "." + name;
        std::string json_key_name = nb::cast<std::string>(key) + "/" + type_name;
        serialized_dict[nb::str(json_key_name.c_str())] =
