...
@runtime_checkable
class Complex64able(Protocol):
    "This protocol describes a type that can be converted to a `complex64`."

    def __complex64__(self) -> Complex64:
        pass


@runtime_checkable
class ImplicitComplex64able(Protocol):
    "This protocol describes a type that can be implicitly converted to a `complex64`."

    def __as_complex64__(self) -> Complex64:
        pass
...
class Complex64:
    ...
    def __init__(self, real, imag=None):
        if isinstance(real, Value):
            assert real.type == T.complex64(
            ), f"Expected complex64, got {real.type}"
            object.__setattr__(self, "value", real)
            return
        if isinstance(real, Complex64):
            object.__setattr__(self, "value", real.value)
            return

        if hasattr(real, '__as_float32__'):
            real = real.__as_float32__().value
        if imag is None:
            imag = constant(0, T.f32())
        elif hasattr(imag, '__as_float32__'):
            imag = imag.__as_float32__().value
        value = complex_dialect.CreateOp(T.complex64(), real, imag).result
    ...
    def __complex128__(self):
        ...
        real = cc.CastOp(T.f64(), real).result
        imag = cc.CastOp(T.f64(), imag).result
        return Complex128(
            complex_dialect.CreateOp(T.complex128(), real, imag).result)
    ...
    def _binary_op(self, other, optor, *, rhs=False):
        if not hasattr(other, '__as_complex64__'):
            return None
        other = other.__as_complex64__()
        if rhs:
            return Complex64(optor(other.value, self.value).result)
        return Complex64(optor(self.value, other.value).result)
    ...
    def real(self):
        ...
        return Float32(complex_dialect.ReOp(self.value).result)

    @property
    def imag(self):
        from .float32 import Float32
        return Float32(complex_dialect.ImOp(self.value).result)
