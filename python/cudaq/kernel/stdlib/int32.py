...
@runtime_checkable
class ImplicitInt32able(Protocol):
...
class Int32:
    ...
    def __init__(self, value: Value | Int32able):
        assert isinstance(
            value,
            (Value,
             Int32able)), f"Expected Value or Int32able, got {type(value)}"
        if isinstance(value, Int32):
            value = value.value
        if isinstance(value, Int32able):
            value = value.__int32__().value
        assert value.type == T.i32(), f"Expected i32, got {value.type}"
    ...
    def __int32__(self) -> Int32:
        return self

    def __int64__(self):
        # TODO: Must give an warning.
        from .int64 import Int64
        return Int64(cc.CastOp(T.i64(), self.value).result)
    ...
    def _binary_op(self, other, optor, *, rhs=False):
        if not isinstance(other, ImplicitInt32able):
            return None
        other = other.__as_int32__()
        if rhs:
            return Int32(optor(other.value, self.value).result)
        return Int32(optor(self.value, other.value).result)
    ...
    def __iadd__(self, other):
        ...
        if isinstance(result, ImplicitInt32able):
            return result.__as_int32__()
        return None
