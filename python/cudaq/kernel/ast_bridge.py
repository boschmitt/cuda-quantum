...
class PyASTBridge(ast.NodeVisitor):
    ...
    def pushValue(self, value):
        ...
        self.valueStack.append(ir.try_downcast(value))
    ...
    def ifPointerThenLoad(self, value):
        if isinstance(value, ir.PointerValue):
            return value.load()
        return value

    def ifNotPointerThenStore(self, value):
        if not isinstance(value, ir.PointerValue):
            slot = ir.alloca(value.type)
            slot.store(value)
            return slot
        return value

    def __copyVectorAndCastElements(self, source, target_element_type):
        if not isinstance(source, ir.StdvecValue):
            raise RuntimeError(
    ...
    def __insertDbgStmt(self, value, dbgStmt):
        ...
        strLitTy = T.ptr(T.array(T.i8(), len(printStr) + 1))
        strLit = cc.CreateStringLiteralOp(strLitTy,
                                          StringAttr.get(printStr)).result
        strLit = cc.CastOp(T.ptr(T.i8()), strLit).result
            ...
            with InsertionPoint(self.entry):
                ...
                for i, b in enumerate(blockArgs):
                    if self.needsStackSlot(b.type):
                        stackSlot = ir.alloca(b.type)
                        ir.store(b, stackSlot)
                        self.symbolTable[argNames[i]] = stackSlot
                    else:
                        self.symbolTable[argNames[i]] = ir.try_downcast(b)
    ...
    def _is_reassignable_value(self, value):
        if isinstance(value, ir.VeqValue):
            return False
    ...
    def visit_Assign(self, node):
        ...
        vars = self.popValue()
        if not isinstance(vars, ir.Tuple):
            vars = ir.Tuple(vars)
        ...
        for var, value_node in zip(vars, values_nodes):
            if isinstance(var, str):
                ...
                if isinstance(value, ir.AbstractValue):
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.ctx, ast.Store):
            self.visit(node.value)
            var = self.popValue()
            if isinstance(var, ir.PointerValue):
        ...
        if isinstance(node.value, ast.Name):
            ...
            if node.value.id in ['np', 'numpy', 'math']:
                ...
                if node.attr == 'complex128':
                    ...
                    return
                if node.attr == 'pi':
                    self.pushValue(ir.constant(np.pi))
                    return
                if node.attr == 'e':
                    self.pushValue(ir.constant(np.e))
                    return
                if node.attr == 'euler_gamma':
                    self.pushValue(ir.constant(np.euler_gamma))
                    return
                raise RuntimeError(
            ...
            if node.value.id == 'cudaq':
                if node.attr in [
                    ...
                    self.pushValue(ir.constant(channel_class.num_parameters))
                    self.pushValue(ir.constant(hash(channel_class)))
                return

        self.visit(node.value)
        value = self.ifPointerThenLoad(self.popValue())

        if isinstance(value, (ir.StructValue, ir.StruqValue)):
            self.pushValue(value.get_field(attr))
            return

        if attr == 'size' and isinstance(value, ir.VeqValue):
            self.pushValue(value.get_length())
            return

        if node.attr in ['imag', 'real']:
            if isinstance(value, ir.ComplexValue):
                if (node.attr == 'real'):
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            ...
            if node.func.id == 'len':
                assert len(args) == 1
                if isinstance(args[0], (ir.StdvecValue, ir.VeqValue)):
            ...
            if node.func.id == 'range':
                self.pushValue(ir.Range(*args))
                return

            if node.func.id == 'enumerate':
                self.pushValue(ir.Enumerate(*args))
                return
            ...
            if node.func.id == 'complex':
                if len(namedArgs) == 0:
                    ...
                    imag = namedArgs['imag']
                    real = namedArgs['real']
                imag = ir.f64_coerce(imag)
                real = ir.f64_coerce(real)
                self.pushValue(
                    complex.CreateOp(T.complex128(), real, imag).result)
                return
            ...
            if node.func.id in [
                ...
                args = [arg.materialize() if isinstance(arg, ir.AbstractValue) else arg for arg in args]
                result = getattr(ir, node.func.id)(args)
                if isinstance(result, ir.Error):
                    self.emitFatalError(result.message, node)
                return

            if node.func.id in ['ch', 'cx', 'cy', 'cz', 'cs', 'ct', 'crx', 'cry', 'crz', 'cr1']:
                args = [arg.materialize() if isinstance(arg, ir.AbstractValue) else arg for arg in args]
                result = getattr(ir, node.func.id[1:]).ctrl(args)
                if isinstance(result, ir.Error):
                    self.emitFatalError(result.message, node)
                return
            ...
            if node.func.id in ['mx', 'my', 'mz']:
                ...
                if len(namedArgs) == 1 and 'register_name' in namedArgs:
                    ...
                    if not isinstance(registerName, ir.StringLiteral):
                        self.emitFatalError(
                ...
                result = getattr(ir, node.func.id)(args, label=registerName)
                if isinstance(result, ir.Error):
                    self.emitFatalError(result.message, node)
            ...
            if node.func.id in globalRegisteredOperations:
                ...
                for i, t in enumerate(args):
                    if not isinstance(t, ir.QRefValue):
                        self.emitFatalError(
            ...
            if node.func.id in globalKernelRegistry:
                ...
                args = [arg.materialize(with_type=fType.inputs[i]) if isinstance(arg, ir.AbstractValue) else arg for i, arg in enumerate(args)]
                ...
                ir.exp_pauli(rotation=args[0], qubits=args[1], pauli_word=args[2])
                return

            elif node.func.id == 'int':
                casted = ir.i64_coerce(args[0])
                if not casted:
                    self.emitFatalError(
                ...
                if isStruq:
                    ...
                    structTy = ir.StructValue.create(structTys,
                                                     name=node.func.id)
            ...
            if node.func.value.id in ['numpy', 'np']:
                ...
                match node.func.attr:
                    case 'complex128':
                        result = ir.complex128_coerce(args[0])
                    case 'complex64':
                        result = ir.complex64_coerce(args[0])
                    case 'float64':
                        result = ir.f64_coerce(args[0])
                    case 'float32':
                        result = ir.f32_coerce(args[0])
                    case 'int32':
                        result = ir.i32_coerce(args[0])
                    case 'int64':
                        result = ir.i64_coerce(args[0])
                    case 'cos':
                        result = ir.cos(args[0])
                    case 'sin':
                        result = ir.sin(args[0])
                    case 'sqrt':
                        result = ir.sqrt(args[0])
                    case 'ceil':
                        result = ir.ceil(args[0])
                    case 'exp':
                        result = ir.exp(args[0])
                    case _:
                        self.emitFatalError(
            ...
            if node.func.value.id == 'cudaq':
                ...
                if node.func.attr == 'qvector':
                    ...
                    if (IntegerType.isinstance(args[0].type)):
                        self.pushValue(ir.VeqValue.create(args[0]))
                        return
                    if isinstance(args[0], ir.StdvecValue):
                        # FIXME: Remove this
                        listScalar = None
                        ...
                        if listScalar != None:
                            ...
                            if not numQubits.is_integer():
                                self.emitFatalError(
                                    ...
                                    node)
                        self.pushValue(ir.VeqValue.from_list(args[0]))
                        return
                    ...
                    if cc.StateType.isinstance(args[0].type):
                        ...
                        self.pushValue(ir.VeqValue.from_state(state_ptr))
                        return
                ...
                if node.func.attr == "qubit":
                    ...
                    self.pushValue(ir.qalloca())
                    return

                if node.func.attr == 'adjoint':
                    args = [arg.materialize() if isinstance(arg, ir.AbstractValue) else arg for arg in args]
                    result = ir.adjoint(args[0], args[1:])
                    if result:
                ...
                if node.func.attr == 'control':
                    args = [arg.materialize() if isinstance(arg, ir.AbstractValue) else arg for arg in args]
                    result = ir.control(symbol_or_value=args[0], controls=args[1], args=args[2:])
                    if result:
                        self.emitFatalError(result.message)
                    return
                ...
                if node.func.attr == 'apply_noise':
                    ...
                    for i, p in enumerate(params):
                        ...
                        if F64Type.isinstance(p.type):
                            alloca = ir.alloca(p.type)
                            ir.store(p, alloca)
                            params[i] = alloca

                    # The remaining arguments are the qubits
                    veq = ir.VeqValue.from_qubits(values[numParams:])
                ...
                if node.func.attr == 'compute_action':
                    ...
                    ir.compute_action(compute=args[0], action=args[1])
                    return
            ...
            if node.func.value.id in ['h', 'x', 'y', 'z', 's', 't', 'rx', 'ry', 'rz', 'r1', 'u3', 'swap']:
                args = [arg.materialize() if isinstance(arg, ir.AbstractValue) else arg for arg in args]
                op = getattr(ir, node.func.value.id)
    ...
    def visit_ListComp(self, node):
        ...
        assert isinstance(iterable, ir.Iterable)
    ...
    def visit_List(self, node):
        ...
        if not is_homogeneous:
            if all(T.is_arithmetic_type(v.type) for v in elements):
                superior_type = T.i64()
                for el in elements:
                    if F32Type.isinstance(el.type):
                        superior_type = el.type
                    if F64Type.isinstance(el.type):
                        superior_type = el.type
                    if ComplexType.isinstance(el.type):
                        superior_type = el.type
                        break  # can do no better

                for i in range(len(elements)):
                    elements[i] = ir.arithmetic_promotion(
                        elements[i], superior_type)
                    assert elements[i]
        ...
        self.pushValue(ir.AList(elements))

    def visit_Constant(self, node):
        if isinstance(node.value, str):
            self.pushValue(ir.literal(node.value))
            return

        if isinstance(node.value, T.InferableType):
            self.pushValue(ir.constant(node.value))
            return
    ...
    def visit_Subscript(self, node):
        ...
        if isinstance(node.ctx, ast.Store):
            ...
            index = self.ifPointerThenLoad(self.popValue())
            ...
            if isinstance(var, ir.VeqValue):
                ...
                return
            if isinstance(var, ir.StdvecValue):
        ...
        if isinstance(node.slice, ast.Slice):
            ...
            if node.slice.lower:
                ...
                lower = self.ifPointerThenLoad(self.popValue())
            if node.slice.upper:
                self.visit(node.slice.upper)
                upper = self.ifPointerThenLoad(self.popValue())

            if isinstance(var, (ir.VeqValue, ir.StdvecValue)):
        ...
        var = self.ifPointerThenLoad(var)
        idx = self.ifPointerThenLoad(self.popValue())

        if not IntegerType.isinstance(idx.type):
            self.emitFatalError(f'invalid index variable type ({idx.type})',
                                node)

        if isinstance(var, ir.AbstractValue):
            var = var.materialize()
        ...
        if isinstance(var, ir.StructValue):
            if var.name != 'tuple':
    ...
    def visit_For(self, node):
        ...
        assert len(self.valueStack) == 1
        iterable = self.popValue()
        iterable = ir.try_downcast(self.ifPointerThenLoad(iterable))
        assert isinstance(iterable, ir.Iterable)
        ...
        def body_builder(index):
            ...
            for target, value in zip(targets, values):
                # FIXME: A target might be a variable that already exists
                if isinstance(value, ir.QRefValue):
    ...
    def visit_While(self, node):
        ...
        def test() -> Value:
            ...
            condition = ir.implicit_conversion(condition, T.i1())
            return condition
        ...
        ir.while_loop(test, body, else_body if len(node.orelse) > 0 else None)
    ...
    def visit_BoolOp(self, node):
        ...
        lhs_condition = ir.implicit_conversion(lhs, T.i1())
        ...
        with InsertionPoint(thenBlock):
            if isinstance(node.op, ast.Or):
                ...
                rhs_condition = ir.implicit_conversion(rhs, T.i1())
    ...
    def visit_Compare(self, node):
        ...
        # TODO: We can proabably do better here.
        if isinstance(left, ir.AbstractValue):
            left = left.materialize()
        if isinstance(right, ir.AbstractValue):
            right = right.materialize()

        result = ir.compare(left, right, op)
        if result:
            self.pushValue(result)
            return
        ...
        if isinstance(op, (ast.In, ast.NotIn)):
            ...
            if not isinstance(right, ir.StdvecValue):
                self.emitFatalError(
                    "Right operand must be a list/vector for 'in' comparison")

            result = right.check_contains(left)
            if isinstance(op, ast.NotIn):
                result = arith.XOrIOp(result, ir.constant(True)).result
            self.pushValue(result)
            return

        self.emitFatalError(f"Unsupported compare operator - {type(op)}", node)

    def visit_AugAssign(self, node):
        self.visit(node.target)
        target = self.popValue()
        assert isinstance(target, ir.PointerValue)

        self.visit(node.value)
        value = self.ifPointerThenLoad(self.popValue())

        loaded_target = target.load()
        result = ir.binary_op(loaded_target, value, node.op)
        if not result:
            self.emitFatalError(
                f"Unsupported binary operator - {type(node.op)}", node)
        result = ir.implicit_conversion(result, loaded_target.type)
        if not result:
    ...
    def visit_Return(self, node):
        ...
        if result.type != self.knownResultType:
            result = ir.implicit_conversion(result, self.knownResultType)
        ...
        if isinstance(result, ir.StdvecValue):
    ...
    def visit_Tuple(self, node):
        ...
        values.reverse()
        self.pushValue(ir.Tuple(*values))
    ...
    def visit_UnaryOp(self, node):
        ...
        if isinstance(node.op, ast.Invert):
            if isinstance(operand, ir.QRefValue):
                self.controlNegations.append(operand)
                self.pushValue(operand)
                return

        result = ir.unary_op(operand, node.op)
        if result:
            self.pushValue(result)
            return
    ...
    def visit_BinOp(self, node):
        ...
        result = ir.binary_op(left, right, node.op)
        if result:
            self.pushValue(result)
            return
        ...
        if node.id in self.capturedVars:
            ...
            if isinstance(value, (list, np.ndarray)) and isinstance(
                ...
                with InsertionPoint.at_block_begin(self.entry):
                    ...
                    elementValues = [ir.constant(el, elements_type) for el in value]
                    ...
                    mlirVal = ir.StdvecValue.from_list(elementValues)
            ...
            if isinstance(value, T.CapturableType):
                ...
                with InsertionPoint.at_block_begin(self.entry):
                    mlirVal = ir.constant(value)
                    stackSlot = ir.alloca(mlirVal.type)
                    ir.store(mlirVal, stackSlot)
            ...
            try:
                if issubclass(value, cudaq_runtime.KrausChannel):
                    ...
                    self.pushValue(ir.constant(value.num_parameters))
                    self.pushValue(ir.constant(hash(value)))
                    return
            except TypeError:
                pass
