...
#
# The `$toolchain` variable indicates which compiler toolchain to build the llvm
# libraries with. We use the same toolchain to buil LLVM and CUDA Quantum.
# Currently, llvm (default), and gcc11 are supported. To use a different
# toolchain, add support for it to the install_toolchain.sh script. If the
# toolchain is set to llvm, then the toolchain will be built from source.
...
RUN apt-get update && apt-get install -y --no-install-recommends git \
    ...
    && git fetch origin --depth=1 $llvm_commit_sha && git reset --hard FETCH_HEAD \
    && apt-get autoremove -y --purge && apt-get clean && rm -rf /var/lib/apt/lists/*
